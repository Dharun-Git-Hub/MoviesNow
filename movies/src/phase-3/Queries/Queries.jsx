  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      ...
      if(data.type === "public_message"){
        setMessage((prev)=>({
          ...prev,
          public: [...(prev.public || [], {from: data.from || "Anonymous", username: data.fromName, text: data.message, self: false})]
        }))
        setMessages((prev) => ({
          ...prev,
          public: [...(prev.public || []), { from: data.from || "Anonymous", username: data.fromName, text: data.message, self: false }],
  ...
  return (
    <div className="chat-app">
      ...
      <div className="main-chat" style={{backgroundColor: selectedUser ? 'white' : "#1f1f1f"}}>
        <span style={{color:'white', position: 'fixed', top: '50%', right: "40%", zIndex: "20", fontSize: "25px"}}>{!selectedUser&&'Support'}</span>
        <div className="chat-header">
          <span className="welcome">{selectedUser ? `${userList.find((el)=>el.id===selectedUser)?.name || selectedUser}` : 'Welcome to Support!'}</span>
        </div>
            ...
            {(messages[selectedUser] || []).map((msg, id1) => (
              <li key={id1} className={msg.self ? 'msg-self' : 'msg-other'}>
                <span>
                  ...
                  {msg.text}
                  {msg.image && (
                    <img src={msg.image} alt="sent" style={{ maxWidth: 200, display: "block", marginTop: 5 }} />
                  )}
                </span>
              </li>
            ))}
        ...
        {selectedUser && <div className="msg-cont">
          <input
            value={message}
          ...
          />
          <button className="send" onClick={() => sendMessage(false)} disabled={message.trim() === ''}>Send</button>
        </div>}
      </div>
    </div>
  );
